...
bot = telebot.TeleBot('7216182705:AAGSeoQebmZbNH9dj8sfwhhEPrMODypqM20')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"CHUDAI start : {target}:{port} for {time}\nSEC Jabtak YE Attack run krrha hai to iske bichme koi Attack nahe Dalna Bhenchod. Feedback Send Kar Laude @PsychoVillain19"
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = "  Pehle BUY krke aa BHENCHOD - @PsychoVillain19 "
